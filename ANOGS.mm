// hook.mm - النسخة النهائية مع تأخير 20 ثانية وواجهة SwiftUI
#import <stdio.h>
#import <string.h>
#import <unistd.h>
#import <stdlib.h>
...
#import <time.h>
#import <errno.h>
#import <dispatch/dispatch.h>   // مهم لـ dispatch_after
#import <Foundation/Foundation.h> // لاستخدام NSClassFromString إلخ
...
static int (*orig_check_provisioning)(void);

// ============================================================================
// Replacement functions – تعطيل آمن بدون كراش
// ============================================================================
...
static int my_access(const char *path, int mode) {
    if (path) {
        const char *sensitive[] = {"Cydia", "MobileSubstrate", "frida", "cydia", "Substrate", "checkra1n", "jailbreak", "apt", "ssh", "debugserver", "proc"};
        for (int i = 0; i < sizeof(sensitive)/sizeof(sensitive[0]); i++) {
            if (strstr(path, sensitive[i])) { errno = ENOENT; return -1; }
        }
    }
...
static int my_check_provisioning(void) { return 0; }

// ============================================================================
// Objective-C swizzling
// ============================================================================
static IMP orig_UIDevice_identifierForVendor;
static id my_UIDevice_identifierForVendor(id self, SEL _cmd) { return [[NSUUID alloc] initWithUUIDString:@"00000000-0000-0000-0000-000000000000"]; }
static void my_LAContext_evaluatePolicy(id self, SEL _cmd, LAPolicy policy, NSString *localizedReason, void(^reply)(BOOL success, NSError *error)) { if (reply) reply(YES, nil); }
...
// ============================================================================
// دوال الفحص الأمني المعدلة (لا تخرج التطبيق)
// ============================================================================
int is_simulator() {
...
void perform_security_checks() { /* لا تفعل شيئاً */ }

// ============================================================================
// Constructor مع تأخير 20 ثانية وواجهة SwiftUI
// ============================================================================
__attribute__((constructor))
void init_hook() {
    srand((unsigned int)time(NULL));
    
    // تأخير 20 ثانية على خيط منفصل (لا يحظر التطبيق)
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(20.0 * NSEC_PER_SEC)), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        // عرض واجهة SwiftUI عبر Bridge في الخيط الرئيسي
